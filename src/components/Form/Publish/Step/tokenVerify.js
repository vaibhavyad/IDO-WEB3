...
export default function TokenVerify({ formProps, blockchain }) {
  ...
  return (
    <s.Container flex={1} ai="center">
      <s.TextTitle className="ido-info-head">Token Verify</s.TextTitle>
      <s.SpacerSmall />
      <TextField
        id="tokenAddress" 
        className="float-input-wrap"
        onChange={(e) => {
          e.preventDefault();
      ...
      {loading ? (
        ...
        ) : <s.TextIDWarning fullWidth className="error">{context.tokenError["token"]}</s.TextIDWarning>
      }
      {data[0] ? (
        <s.Container className="input-field" style={{}}>
          <s.SpacerSmall />
